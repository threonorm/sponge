...
module TPTPBackend : BACKEND = struct
  ...
  let create () =
   (* Generate a new file name *)
    ...
    let query_file_path = temp_file ~temp_dir:egg_repo_path "tptp_" ".in" in
    ...
    let response_file_path = temp_file ~temp_dir:egg_repo_path "tptp_" ".out" in
  ...
  let prove t a =
    ...
    flush t.oc;
    let command = "cd \"" ^ egg_repo_path ^ "\" && ./egg-sc-tptp " ^t.query_file_path ^ " " ^ t.response_file_path ^ " --level1" in
    (* printf.printf "%s\n" command; *)
    let status = Sys.command command in
    if log_misc_tracing() then Printf.printf "command '%s' returned exit status %d\n" command status else ();
    
    Some([t.response_file_path])
...
let rec build_sequence_refines last_name res  env sigma= 
  let (name,step) = List.find (fun (x,y) -> x = last_name) res in
  match step with 
  | Cut(c) ->
    let (sigma,constr) = Constrintern.interp_constr_evars env sigma (parse_constr_expr c.name_theorem) in
    let id = Names.Name.mk_name (Names.Id.of_string c.name_in_context) in
    tclBIND (Tactics.pose_proof id constr) 
      (fun _noVal ->
        build_sequence_refines c.and_then res env sigma)
  | LeftForall(l) -> 
    Goal.enter (fun gl -> 
    let env = Goal.env gl in
    let sterm = ("(" ^ l.to_specialize ^ " " ^ l.term ^ ")") in
    let (sigma,c) = Constrintern.interp_constr_evars env sigma (parse_constr_expr sterm) in
    tclBIND (Tactics.specialize (c, Tactypes.NoBindings) None)
      (fun _noVal -> 
      build_sequence_refines l.and_then res env sigma))
  | RightRefl -> 
    Tactics.reflexivity
  | RightSubst(s) ->
    Goal.enter (fun gl -> 
    let env = Goal.env gl in
    let rw_str = ((if s.backward then "(@rew_zoom_bw _ _ _ " else "(@rew_zoom_fw _ _ _ " )^ s.rw ^ " (" ^ s.focus ^ "))") in
    (* Printf.printf "Rewrite Generated%s\n" rw_str; *)
    let (sigma,constr) = Constrintern.interp_constr_evars env sigma (parse_constr_expr rw_str) in
    let hid =  Names.Id.of_string s.rw in
    tclBIND (
       (* tclUNIT ()  *)
      Tactics.eapply constr
      )
      (fun _noVal -> 
      tclBIND (
        tclUNIT()
        (* Proofview.tclDISPATCH [Tactics.reflexivity; tclUNIT()] *)
        (* CLEAR *)
      )
      (fun _noVal -> 
        tclBIND (Tactics.clear [hid]) (fun _noVal ->
        Goal.enter (fun gl -> 
        let env = Goal.env gl in
        build_sequence_refines s.and_then res env sigma)
        (* tclUNIT() *)
        ))))
...
let egg_tptp () =
  Goal.enter begin fun gl ->
    ...
    match B.prove b (AProp g) with 
    | Some([f]) ->
      let res = generate_proof f in
      let (last_name,_) = List.nth res (List.length res - 1) in
      B.close b;
      build_sequence_refines last_name res env sigma
